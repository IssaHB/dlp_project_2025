...
#
# MODIFIED BY: Isa Chousein Basia
# DATE: 2025-04-04
# CHANGES: Added grid lines and labels to the plot, as well as a parameter
# to rotate the x_ticks in the confusion matrix by a desired angle.
...
def plot_confusion_matrix(y_true, y_pred, classes=None, normalize=False, title=None, cmap=plt.cm.Blues, x_tick_rot=0, dpi=100):
    """
    ...
    if classes is not None:
        ...
        plt.xticks(tick_marks, classes, rotation=x_tick_rot)
...
def plot_model_history(history, dpi=100):
    ...
    fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(10, 5), dpi=dpi)
    ax[0].plot(np.arange(n_epochs), loss, label="Training", color='tab:blue')
    ax[0].plot(np.arange(n_epochs), val_loss, label="Validation", color='tab:orange')
    ax[0].legend()
    ax[0].set_xlabel("Epoch")
    ax[0].set_ylabel("Loss")
    ax[0].set_xlim([0,n_epochs-1])
    ax[0].set_ylim([0,max(max(loss), max(val_loss))+0.05])
    ax[0].grid(alpha=0.4)

    ax[1].plot(np.arange(n_epochs), acc, label="Training", color='tab:blue')
    ax[1].plot(np.arange(n_epochs), val_acc, label="Validation", color='tab:orange')
    ax[1].legend()
    ax[1].set_xlabel("Epoch")
    ax[1].set_ylabel("Accuracy")
    ax[1].set_xlim([0,n_epochs-1])
    ax[1].set_ylim([0,max(max(acc), max(val_acc))+0.05])
    ax[1].grid(alpha=0.4)
